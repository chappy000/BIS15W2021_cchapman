...
homerange %>% 
  ...
  geom_histogram(fill="cornflowerblue", color="steelblue", alpha = 0.5)+
...
homerange %>% 
  ggplot(aes(x = log10.hra))+
  geom_density(fill="cornflowerblue", color="steelblue", alpha = 0.5)+
  labs(title = "Distribution of Homerange Size")
```
...
homerange %>% 
  ggplot(aes(x = log10.hra))+
  geom_histogram(aes(y=..density..), fill="cornflowerblue", color="steelblue", alpha = 0.5)+
...
Use the quartiles to make ranges for new categories
```{r}
summary(homerange$log10.mass)
```

Make new categorical variable called mass_category
