...
```{r}
my_palette <- paletteer_d("LaCroixColoR::MelonPomelo")
```
...
Helpful for looking at outliers
```{r}
homerange %>% 
...
Don't wrap with a variable that has many categories
```{r}
p2+ facet_wrap(~taxon)
```
...
## Printing
"pdf" is a device, dev.off closes the device
```{r}
pdf("homerange.pdf")
pd
dev.off()
