...
Only joined the customer ID's in common
```{r}
inner_exampleDF <- inner_join(table_A, table_B, by="customer_ID")
inner_exampleDF
```
Well introduce NAs when not doing inner join, when not using only data in common

`left_join`
Where did customer 4 go? Notice the missing data for customer 3.
Used the customer ID in the left table, the first table
```{r}
...
Used the customer IDs in the right table, the first table
```{r}
...
`full_join`
Used all customer IDs in both tables
```{r}
