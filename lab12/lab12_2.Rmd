...
f = expands boundaries a little bit beyond your coordinates
```{r}
lat <- c(34.67, 41.80)
long <- c(-124.1, -115.5)
...
```{r}
ggmap(map2) + 
  geom_point(data = spiders, aes(longitude, latitude), alpha = 0.5, color = "darksalmon", size = 1.25) +
  labs(x = "Longitude", y = "Latitude", title = "Spider Locations")
```
...
Size = determines the boundaries
```{r}
ggplot() + 
