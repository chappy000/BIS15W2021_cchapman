...
```{r}
ui <- fluidPage(titlePanel("Homerange Locomotion"), radioButtons("fill", "Select Fill", choices = c("trophic.guild", "thermoregulation"), selected = "trophic guild"),
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    ggplot(homerange, aes_string(x = "locomotion", fill = input$fill)) + geom_bar(position = "dodge", alpha = 0.5) + theme_minimal()
})
